...
const formatMovimentation = (movimentation: IMovimentation): any => {
  movimentation.date = new Date(movimentation.date);
...
const deleteMovimentation = async (
  ...
  try {
    const deleteMovimentationApiUrl = `${apiUrl}/${movimentationPrefix}/${id}`;
...
export { addMovimentation, getMovimentation, deleteMovimentation };
