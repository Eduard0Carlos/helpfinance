...
const BarChart = (props: IBarChartProps) => {
  let width = ((props.value / props.total) * 100).toFixed(0);

  width = new Number(width) as number > 100 ? "100" : width;
  ...
  const getBuildinBarChart = () => (
    <div className={"bar-chart build-in"}>
      <div className="bar-chart-bars">
        ...
        <rect className="background" />
        <div className="infos">
          <h1 className="current-value" style={(new Number(width) as number) > 10 ? {color: "#333"} : {}}>{formatMoney(props.value)}</h1>
          <h1 className="porcentage" style={(new Number(width) as number) > 45 ? {color: "#333"} : {}}>{width}%</h1>
          <h1 className="total-value" style={(new Number(width) as number) > 80 ? {color: "#333"} : {}}>{formatMoney(props.total)}</h1>
        </div>
      </div>
    </div>
  );
